...
http
  .createServer((req, res) => {
    ...
    if (req.url === "/health") {
      ...
      res.end(
        JSON.stringify({
          status: botConnected ? "connected" : "waiting",
          qr: !!currentQR,
        })
      );
      return;
    ...
    if (botConnected) {
      res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Online Gulio Bot v3</title></head> <body style="font-family:Arial;text-align:center;padding:50px;background:#f5f5f5;"> <h1 style="color:#E85D04">🏪 Online Gulio Bot v3</h1> <div style="background:#22C55E;color:white;padding:20px;border-radius:12px;font-size:22px;max-width:400px;margin:20px auto;"> ✅ Bot Imeunganishwa!<br><small style="font-size:14px;opacity:.8;">Sellers wanaweza kutuma bidhaa sasa</small> </div> <p style="color:#666;font-size:13px;">Session imehifadhiwa kwenye Firestore — haitahitaji QR tena baada ya restart.</p> </body></html>`);
    } else if (currentQR) {
      res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Scan QR — Online Gulio Bot</title> <script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script> <meta http-equiv="refresh" content="25"> </head><body style="font-family:Arial;text-align:center;padding:30px;background:#fff;"> <h1 style="color:#E85D04">🏪 Online Gulio Bot v3</h1> <h2>Scan QR Code — Mara Moja Tu!</h2> <p style="color:#666">WhatsApp → Linked Devices → Link a Device</p> <div id="qr" style="margin:20px auto;display:inline-block;padding:20px;border:3px solid #E85D04;border-radius:12px;"></div> <p style="color:#999;font-size:12px;">Baada ya scan moja, session itahifadhiwa. Haitahitajika tena! ✅</p> <p style="color:#999;font-size:12px;">Ukurasa unabadilika kila sekunde 25.</p> <script>new QRCode(document.getElementById("qr"),{text:${JSON.stringify( currentQR )},width:256,height:256});</script> </body></html>`);
    } else {
      res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="5"><title>Online Gulio Bot v3</title></head> <body style="font-family:Arial;text-align:center;padding:50px;"> <h1 style="color:#E85D04">🏪 Online Gulio Bot v3</h1> <p>⏳ Inaanza... subiri sekunde chache.</p> </body></html>`);
    }
  })
  .listen(PORT, () => {
...
async function findSellerByWA(phone) {
  try {
    ...
    if (!snap.empty) {
      ...
      console.log(`✅ Seller found: ${user.name}`);
      return { uid: snap.docs[0].id, ...user };
    }
    const altClean = clean.startsWith("255")
      ? "0" + clean.slice(3)
      : "255" + clean.slice(1);
  ...
  sock.ev.on(
    ...
    async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === "close") {
        ...
        console.log(
          `Connection closed (code: ${code}). Reconnect: ${shouldReconnect}`
        );
        if (shouldReconnect) setTimeout(startBot, 5000);
        else
          console.log(
            "⚠️ Logged out — clear bot_auth collection in Firestore and restart"
          );
      }
      if (connection === "open") {
        botConnected = true;
