...
async function findSellerByWA(phone) {
  try {
    ...
    const snap2 = await db.collection('users').where('waNumber', '==', altClean).limit(1).get();
    if (!snap2.empty) {
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      else console.log('⚠️  Logged out — clear bot_auth collection in Firestore and restart');
    }
    if (connection === 'open') {
      botConnected = true;
      currentQR = null;
